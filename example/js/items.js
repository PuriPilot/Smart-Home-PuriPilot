...
$(document).ready(function() {
  ...
  for (var i = 0; i < items.length; i++) {
    var item = items[i];
    var deviceAttr = item.deviceId ? (' data-device-id="' + item.deviceId + '"') : '';
    var deviceTemplateAttr = item.deviceTemplate ? (' data-device-template="' + item.deviceTemplate + '"') : '';
    var html = '<div class="col-sm-4">' +
                ...
                '" model-type="' +
                item.type + 
          '"' + deviceAttr + deviceTemplateAttr + '><img src="' +
                item.image + 
                '" alt="Add Item"> '+
                item.name +
                '</a></div>';
